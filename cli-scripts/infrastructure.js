...
function ${entityName}Repository(): I${entityName}Repository {
  return {
    find: () =>
      serviceHandler<${entityName}>(() => request.get(endpoints.${entityName.toUpperCase()}.GET_${entityName}())),

    update: (body: ${entityName}UpdateParams) =>
      serviceHandler<${entityName}Update>(() =>
        request.put(endpoints.${entityName.toUpperCase()}.PUT_${entityName}(), body)
      ),
...
function _restfulRepositoryInterface(entity) {
  return `
import ${entity}RestfulRepository from './restful/${entity}RestfulRepository';
